...
exports.MAPS = exports.HOUSE_IDS = void 0;
const createTile = (type, walkableOverride) => ({
    type,
...
exports.HOUSE_IDS = Array.from({ length: HOUSE_COUNT }, (_, index) => String(index + 1).padStart(2, '0'));
const HOUSE_POSITIONS = exports.HOUSE_IDS.map((houseNum, index) => ({
    houseNum,
...
const createNPC = (id, x, y, spriteId = 'npc-default') => ({
    id,
    name: id.replace('-', ' '),
    position: { x, y },
    spriteId,
...
const buildValeVillageNPCs = () => [
    createNPC('tower-attendant', TOWER_ENTRANCE_COLUMN, ROAD_ROW),
];
const buildValeVillageTriggers = () => {
    const houseTriggers = HOUSE_POSITIONS.map(({ houseNum, overworldX }, index) => ({
        ...
        data: {
            ...
            requiredFlags: index === 0 ? undefined : [`house-${exports.HOUSE_IDS[index - 1]}`],
        },
    }));
    return [
        ...houseTriggers,
...
const createHouseInteriorTiles = () => {
    const tiles = [];
...
const createHouseInterior = (houseId, houseNum, overworldEntranceX) => {
...
const HOUSE_MAPS = HOUSE_POSITIONS.reduce((maps, { houseNum, overworldX }) => {
