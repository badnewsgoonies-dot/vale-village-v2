...
exports.getElementModifier = getElementModifier;
exports.calculateTotalEquipmentElementalResistance = calculateTotalEquipmentElementalResistance;
exports.applyDamageModifiers = applyDamageModifiers;
exports.calculatePhysicalDamage = calculatePhysicalDamage;
exports.calculatePsynergyDamage = calculatePsynergyDamage;
exports.calculateHealAmount = calculateHealAmount;
exports.isInvulnerable = isInvulnerable;
exports.hasShieldCharges = hasShieldCharges;
exports.consumeShieldCharge = consumeShieldCharge;
exports.applyDamage = applyDamage;
exports.applyDamageWithShields = applyDamageWithShields;
exports.checkAutoRevive = checkAutoRevive;
exports.applyHealing = applyHealing;
 ...
 */
function getElementModifier(attackElement, defenseElement) {
    const key = `${attackElement}→${defenseElement}`;
    if (ELEMENT_ADVANTAGE[key]) {
        return constants_1.BATTLE_CONSTANTS.ELEMENT_ADVANTAGE_MULTIPLIER; // +50% damage
    }
    const reverseKey = `${defenseElement}→${attackElement}`;
    if (ELEMENT_ADVANTAGE[reverseKey]) {
        return constants_1.BATTLE_CONSTANTS.ELEMENT_DISADVANTAGE_MULTIPLIER; // -33% damage
    }
    return 1.0; // Neutral
}
/**
 ...
 */
function calculateTotalEquipmentElementalResistance(unit) {
 ...
 */
function applyDamageModifiers(baseDamage, abilityElement, defender) {
    ...
    const damageReductionEffects = defender.statusEffects.filter(effect => effect.type === 'damageReduction');
 ...
 */
function calculatePhysicalDamage(attacker, defender, team, ability) {
    const attackerEffective = (0, stats_1.calculateEffectiveStats)(attacker, team);
    const defenderEffective = (0, stats_1.calculateEffectiveStats)(defender, team);
    ...
    const rawDamage = baseDamage + attackPower - (effectiveDefense * constants_1.BATTLE_CONSTANTS.DEFENSE_MULTIPLIER);
    ...
    const damage = Math.max(constants_1.BATTLE_CONSTANTS.MINIMUM_DAMAGE, Math.floor(modifiedDamage));
    return damage;
}
/**
 ...
 */
function calculatePsynergyDamage(attacker, defender, team, ability) {
    const attackerEffective = (0, stats_1.calculateEffectiveStats)(attacker, team);
    const defenderEffective = (0, stats_1.calculateEffectiveStats)(defender, team);
    ...
    const ignoreDefensePercent = Math.min(1, Math.max(0, ability.ignoreDefensePercent || 0));
    const effectiveDefense = defenderEffective.def * (1 - ignoreDefensePercent);
    const magicDefense = effectiveDefense * constants_1.BATTLE_CONSTANTS.PSYNERGY_DEFENSE_MULTIPLIER;
 ...
 */
function calculateHealAmount(caster, team, ability) {
    ...
    const casterEffective = (0, stats_1.calculateEffectiveStats)(caster, team);
    const magicPower = casterEffective.mag;
    const rawHeal = baseHeal + magicPower;
    const healAmount = Math.max(constants_1.BATTLE_CONSTANTS.MINIMUM_HEALING, Math.floor(rawHeal));
    return healAmount;
}
/**
 ...
 */
function hasShieldCharges(unit) {
    return unit.statusEffects.some(effect => effect.type === 'shield' && effect.remainingCharges > 0);
}
/**
 ...
 */
function consumeShieldCharge(unit) {
 ...
 */
function applyDamageWithShields(unit, damage) {
    ...
    updatedUnit = {
        ...updatedUnit,
        statusEffects: updatedUnit.statusEffects.filter(effect => !(effect.type === 'shield' && effect.remainingCharges === 0)),
    };
    // 4. Check for auto-revive if unit is KO'd
 ...
 */
function checkAutoRevive(unit) {
    // Only trigger if unit is KO'd
    if (unit.currentHp > 0) {
        // Clean up exhausted auto-revive statuses even if unit isn't KO'd
        const hasExhaustedAutoRevive = unit.statusEffects.some(effect => effect.type === 'autoRevive' && effect.usesRemaining === 0);
        if (hasExhaustedAutoRevive) {
            const cleanedStatusEffects = unit.statusEffects.filter(effect => !(effect.type === 'autoRevive' && effect.usesRemaining === 0));
    ...
    const autoReviveEffects = unit.statusEffects.filter(effect => effect.type === 'autoRevive' && effect.usesRemaining > 0);
 ...
 */
function applyHealing(unit, healing, abilityRevivesFallen = false) {
    ...
    if ((0, Unit_1.isUnitKO)(unit) && !abilityRevivesFallen) {
