...
export class RoadLayer implements Layer {
  zIndex = 2; // Between BackgroundLayer (1) and VillageLayer/TerrainLayer (3+)

  private readonly ROAD_Y_TOP = 420;
  private readonly ROAD_Y_BOTTOM = 480;
  private readonly ROAD_HEIGHT = this.ROAD_Y_BOTTOM - this.ROAD_Y_TOP; // 60px
    ...
    for (let x = Math.floor(worldLeft / spacing) * spacing; x < worldLeft + roadWidth; x += spacing) {
      for (let y = 0; y < this.ROAD_HEIGHT; y += spacing) {
        ...
        ctx.fillRect(
          screenX,
          stoneY,
          stoneSize + random(x * 23 + y * 29) * 4,
          stoneSize + random(x * 29 + y * 23) * 4
        );
      }
    }
